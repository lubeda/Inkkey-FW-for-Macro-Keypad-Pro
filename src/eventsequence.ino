...
void defaultAssignment() {
  ...
  for (byte key = 0; key < 6; key++) {
    //F1 to F8 on buttons
    assignments[key][0][0].deviceAndType = DEVICE_KEYBOARD | TYPE_PRESS;
    assignments[key][0][0].keycodeOrDelay = KEY_F13 + key ;
    assignments[key][1][0].deviceAndType = DEVICE_KEYBOARD | TYPE_RELEASE;
    assignments[key][1][0].keycodeOrDelay = KEY_F13 + key ;
  }
  //Left click on rotary press
  assignments[6][0][0].deviceAndType = DEVICE_MOUSE | TYPE_PRESS;
  assignments[6][0][0].keycodeOrDelay = (MOUSEAXIS_BUTTON << 8) | MOUSE_LEFT;
  assignments[6][1][0].deviceAndType = DEVICE_MOUSE | TYPE_RELEASE;
  assignments[6][1][0].keycodeOrDelay = (MOUSEAXIS_BUTTON << 8) | MOUSE_LEFT;
  
  //Mouse wheel on rotary
  assignments[7][0][0].deviceAndType = DEVICE_MOUSE | TYPE_INCREMENT;
  assignments[7][0][0].keycodeOrDelay = (MOUSEAXIS_WHEEL << 8) | (1 & 0xff);
  assignments[7][1][0].deviceAndType = DEVICE_MOUSE | TYPE_INCREMENT;
  assignments[7][1][0].keycodeOrDelay = (MOUSEAXIS_WHEEL << 8) | (-1 & 0xff);
}
